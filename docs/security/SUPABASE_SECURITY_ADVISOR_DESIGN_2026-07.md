...
Final manual staging Supabase Security Advisor rerun reported `0` errors, `1` warning, and `9` info suggestions. The only remaining warning is authenticated execution of `public.is_current_user_admin()`. That warning is accepted temporarily for this release because anonymous/public execute has been revoked, authenticated execute is needed by current RLS policies, and the helper returns only a boolean. A deeper helper redesign is deferred to a later hardening sprint.

The `9` info suggestions are deferred and are not blocking this security release unless later review proves one affects the approved access contracts. The staging DB gate is passed. Remaining validation work is outside SQL migration proof: deploy web commit `5006e1e` to a preview/staging host and complete browser smoke tests. Production remains blocked pending production ledger review and an approved migration repair or current-timestamp wrapper strategy. Staging ledger quirks around older date-only `20260507` / `20260509` migrations must be documented before using staging as a durable release environment.
