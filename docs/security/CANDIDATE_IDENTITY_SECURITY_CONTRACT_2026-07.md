...
The app repo now owns that contract in `supabase/migrations/20260708000000_reconcile_web_publication_and_employer_contracts.sql`. The migration preserves the candidate-facing rule that anonymous users can only read rows where `status = 'published'`.
