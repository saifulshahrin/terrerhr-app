...
comment on column public.external_opportunities.source_reference_id is
  'Immutable internal source metadata for reconciliation; not an enforced cross-company identity.';
...
create index external_opportunities_source_reference_reconciliation_idx
  on public.external_opportunities (
    source_type,
    lower(source_name),
    source_reference_id
  )
  include (company_name, id)
